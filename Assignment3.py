...
def get_function_type(func, func_domain, func_co_domain):
    ...
    return (f"is_total {is_total}, is_partial {is_partial}, is_injective {is_injective}, is_surjective {is_surjective},"
            f" is_bijective {is_bijective}")
...
# XxY = {[a,a], [ab,a], [abc,a], [abcd,a], [a,bb], [ab,bb], [abc,bb], [abcd,bb],  [a,ccc], [ab,ccc],
#        [abc,ccc], [abcd,ccc], [a,dddd], [ab,dddd], [abc,dddd], [abcd,dddd]}
# P(X) = {{}, {a}, {ab}, {abc}, {abcd}, {a,ab}, {a,abc}, {a,abcd}, {ab,abc}, {ab, abcd}, {abc, abcd},
#         {a,ab,abc}, {a,ab,abcd}, {a,abc,abcd}, {ab,abc,abcd}, {a,ab,abc,abcd}}
...
# [c] Compare the results (since there may be round off error,
...
def sum_cubes_formula(n):
    return (n * (n + 1) / 2) ** 2  # Square of the sum of counting numbers


def sum_geometric_loop(n, a, r):  # r is ratio
...
def sum_arithmetic_loop(n, a, d):  # d is difference
...
# fractional binary numbers in the range [0,1) and uses Cantor's diagonalization argument to find a
# number not already in that list. Be mindful of the case where there are more elements in the list than digits
# in each number, and assume implicit trailing zeros as needed.
#
...
def generate_binary_number(numbers):
    ...
    # we turned 0 into 1 i+2 skips "0."


def convert_to_decimal(number):
    return sum([int(number[i]) / 2**(i - 1) for i in range(2, len(number))])  # Now starting at position 2
