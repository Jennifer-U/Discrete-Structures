...
def compute_values(funcs, inputs):  # helper function computes values
    dict_f = {}
    for func in funcs:
...
# [1] Define a function to parse a linear homogeneous recurrence and extract both its initial conditions
# and coefficients
def get_sides(eqn):
    parts = eqn.split("=")
...
def solve_recurrence(desc, c1, c0, a0, a1):
    ...
    formula = ("a(n) = " + str(X[0]) + "*" + str(r1) + "^n" + " + " + str(X[1]) + "*" + ("n*" if not distinct else "") +
               str(r2) + "^n")
...
def process_recurrence(data, desc, recurrence, n, func, verbose=False):
    ...
    if verbose:
        ...
        print("old_recurrence", recurrence)
        print("new_recurrence", new_recurrence)
        print("characteristic equation is", characteristic_equation)
...
def prepare_output(data, n, ranked=False):
    ...
    title = f"Linear Homogeneous Recurrences for n={n}"
    heads = ["Name", "Description", "Recurrence", "New Recurrence",
             "Function Syntax", "Characteristic Equation", "Formula",
             "Value", "Log Value"]
...
# as per request Extra Example Summer 2022 Exam
def f7(f, n):
    return 0 if n == 0 else (1 if n == 1 else 6*ff(f7, n-1) - 5 * ff(f7, n-2))
...
def main():
    ...
    recurrences = [
        ...
        ("Summer2 2023 Exam", "f(0) = 6, f(1) = 7, f(n) = 9f(n-1)-8f(n-2)", f6),
        ("Extra Ex Summer 2022 Exam", "f(0) = 0, f(1) = 1, f(n) = 6f(n-1)-5f(n-2)", f7)
    ]
    ...
    dict_funcs2 = compute_values(funcs, inputs)
