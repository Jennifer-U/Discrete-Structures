...
# including the evaluated expression, followed by your categorization of the
# expression as a tautology, contingency, or contradiction.
...
# [8] Apply the analyze function to some logical equivalences,
# the two parts of De Morgan's Laws.
# Law #1: ~(p | q) ↔ (~p & ~q)
# Law #2: ~(p & q) ↔ (~p | ~q)
...
# [10] Define a function is_equivalent(f1, f2) that determines if two functions are logically equivalent
# by building the truth tables for both and comparing outputs, or by evaluating f1 ↔ f2 to be a tautology.
...
# Returns not only the type (tautology / contingency / contradiction)
# but also any other data of interest: rows, columns, variables,
# output column, binary equivalent, and decimal equivalent
# Define a function is_equivalent(f1, f2) that determines if two functions are
# logically equivalent by building the truth tables for both and comparing outputs,
# or by evaluating f1 ↔ f2 to be a tautology. Test that it works as expected.
...
def main():
    ...
    functions = [f0_false, f1_nor, f2_converse_nonimpl, f3_neg_p, f4_nimply, f5_neg_q,
                 f6_xor, f7_nand, f8_and, f9_xnor, f10_q, f11_imply, f12_p,
                 f13_converse, f14_or, f15_true]
