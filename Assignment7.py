...
def sum_geometric_formula(n, a, r):
    return n + 1 if r == 1 else int(round(a * ((r ** (n + 1) - 1) / (r - 1))))
...
# using the formula for f(n) of the
...
# How many strings do you expect there to be?
# the number of strings of length <= m over an alphabet with n characters is (n^(m+1) - 1) / (n - 1)
